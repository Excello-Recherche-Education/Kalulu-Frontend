...
func play_syllable(syllable: Dictionary) -> void:
	...
	var syllable_audiostream: AudioStream = Database.load_external_sound(Database.get_syllable_sound_path(syllable))
...
func play_word(word: String) -> void:
	...
	var word_audiostream: AudioStream = Database.load_external_sound(Database.get_word_sound_path({Word = word}))
