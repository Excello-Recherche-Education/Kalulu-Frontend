...
const description_line_class: PackedScene = preload("res://sources/language_tool/video_gp_description.tscn")


func _ready() -> void:
	var _description_line: VideoGPDescription = description_line_class.instantiate()
	...
	for gp in Database.db.query_result:
		var description_line: VideoGPDescription = description_line_class.instantiate()
